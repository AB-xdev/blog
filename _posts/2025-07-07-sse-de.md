...
layout: post
title: |
  Ein Blick in Extras for Spring Security (SSE)
categories:
  - Blog
tags:
  - opensource
...
Vor einiger Zeit haben wir _[Extras for Spring Security](https://github.com/xdev-software/spring-security-extras)_ (SSE) veröffentlicht, um die Absicherung unserer Anwendungen zu vereinfachen.

Dieser Beitrag gibt einen Einblick in den Code und stellt einige Features vor, die auch für dich interessant sein könnten.

## Übersicht

SSE bietet verschiedene Module, die es einfacher und sicherer machen, verschiedene Sicherheitslösungen in Spring-Anwendungen zu erstellen/verwalten, insbesondere in verteilten Systemen.
...
Die Gesamtlösung ist ganz ok, allerdings gibt es ein paar Probleme:
* Die Reaktion auf einen [manually implement responding to a Back-Channel logout request](https://docs.spring.io/spring-security/reference/reactive/oauth2/login/logout.html#configure-provider-initiated-oidc-logout) muss manuell implementiert werden, was bei verteilten Systemen (Cluster) komplex sein kann.
...
SSE macht sich genau dies zunutze, indem es regelmäßig überprüft, ob der Benutzer noch Zugang hat.<br/>
...
| Feature | BackChannel-basierte Abmeldung | AccessToken-Revalidierung mit SSE |
| --- | --- | --- |
| Implementierung | Manual implementation required | SSE stellt vordefinierte Klassen zur Verfügung; minimaler Aufwand |
...
Bitte beachten Sie, dass es auch möglich ist, beide Mechanismen zu kombinieren und das Beste aus beiden Welten zu nutzen.
...
Dafür stellt SSE einen ``ReloadCommunicator`` zur Verfügung, der verwendet werden kann, um dem Frontend mitzuteilen, dass eine bestimmte Aktion erforderlich ist.
...
Wenn man nur 1 Identitätsanbieter in Spring Boot hat? Kein Problem, der Benutzer wird automatisch zu diesem Provider umgeleitet.
...
Aber wie wäre es, sie direkt zu ihrem letzten Anbieter umzuleiten? Genau das ist in der SSE mit [ein paar Zeilen Code](https://github.com/xdev-software/spring-security-extras/tree/develop/oauth2-oidc#rememberme-login-provider-redirect) möglich.
...
* nachdem er/der Server neu gestartet wurde
...
SSE bietet die folgende Lösung an, die die meisten dieser Probleme behebt:
...
SSE bietet einen Mechanismus zur Registrierung öffentlich verfügbarer zustandsloser Ressourcen, wie statische Dateien. Dadurch wird verhindert, dass Sitzungen erstellt werden, wo sie nicht benötigt werden, zum Beispiel für ``robots.txt``.
...
SSE ist als OpenSource-Software [auf GitHub](https://github.com/xdev-software/spring-security-extras) einsehbar.<br/>Wenn Ihnen der Artikel gefallen hat, können Sie dem Repository gerne einen Stern ⭐ geben.
